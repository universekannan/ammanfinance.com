...
function calculate_amount(e,id){
...
function validate_amount(e,id){
      ...
      payamount = Number(payamount);
      creditamount = Number(creditamount);
      if(payamount < interestamount){
         //alert("Minimum Pay Amount is "+interestamount);
    ...
    $("#giveamound").keyup(function(){
        var getval = this.value;
        var bal = $('#measurement').val();
        if(bal < getval) {
            $("#error").css("display", "block");
            $('#credit_amount').val(0); 
        } else {
           var res = bal * getval;
            $('#credit_amount').val(res); 
