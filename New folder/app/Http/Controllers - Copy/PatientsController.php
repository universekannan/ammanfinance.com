...
/******   Manage Users  Start ******/
        public function managePatients(){
			...
			$manageoppatients = DB::table('patients')->select('patients.*','patient_disease.*','patients.id as userID')
                ...
                ->where('patients.role_id', '=','4')->where('patients.status', '=','2')->orderBy('patients.id','Asc')->get();
				
			$manageippatients = DB::table('patients')->select('patients.*','patient_disease.*','patients.id as userID')
                ->Join('patient_disease', 'patient_disease.id', '=', 'patients.disease_id')
                ->where('patients.role_id', '=','4')->where('patients.status', '=','3')->orderBy('patients.id','Asc')->get();
				
            $managedoctor = DB::table('users')->where('role_id', '=','2')->orderBy('id','Asc')->get();
		
            return view("patients.index")->with('manageoppatients', $manageoppatients)->with('manageippatients', $manageippatients)->with('managedoctor', $managedoctor);
        }	
      ...
      //  public function managePatients(){
          ...
          //  return view("users.index")->with('managepatients', $managepatients);
      //  }
