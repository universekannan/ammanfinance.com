...
@section('content') 
                   ...
                   <div class="card-body">
			       <form action="{{url('/pharmacy/add_locations')}}" method="post">
                            {{ csrf_field() }}
                   <div class="modal-header">
                   <h4 class="modal-title">Rack Location</h4>
                   </div>
                   <div class="modal-body">					
                   <input type="text" class="col-sm-6" name="rack_name" 
		   ...
		   @foreach($manageLocations as $key=>$manageLocations)
                   <tr id="arrayorder_<?php echo $manageLocations->id?>">
                   <td>{{ $key + 1 }}</td>
                   <td>{{ $manageLocations->rack_name }}</td>
   					@if($manageLocations->status == 0)
